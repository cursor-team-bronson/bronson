...
#   emit_artifacts uses shell + Node stdin only (no workspace_write). If you set
#   ALLOW_WORKSPACE_WRITE=false globally, dream-state still runs.
#
...
jobs:
  ...
  draft_agent_workflows:
    prompt: |
      ...
      UI schedule preference key bronson.workflowSchedule.v1 for runners that POST on a cadence.
  ...
  emit_artifacts:
    prompt: |
      You only have the shell tool (no workspace_write). Write UTF-8 files under TOOL_SHELL_CWD using Node.js.
      Pass large bodies via the shell tool stdin field — never inline megabytes in the command string.

      Writer pattern (repeat per file, change OUT path and stdin body):

        command:
          node -e "const fs=require('fs');const p=require('path');const out='REL_PATH';fs.mkdirSync(p.dirname(out),{recursive:true});let b='';process.stdin.on('data',d=>b+=d);process.stdin.on('end',()=>fs.writeFileSync(out,b));"
        stdin: <exact bytes for that file>

      Use forward slashes in REL_PATH (e.g. .dream/memory-output.md, .dream/proposed/memory-hygiene.yaml).
      ...
      Steps:
        1) Write .dream/memory-output.md — stdin = consolidate Markdown only.

        2) Parse draft_agent_workflows; extract raw YAML between MEMORY_HYGIENE markers (if present).
           Write .dream/proposed/memory-hygiene.yaml via stdin.

        3) Extract YAML between CAPABILITY markers; write .dream/proposed/capability-build.yaml.

        4) Write .dream/proposed/README.md (8–12 lines): what each artifact is, how to POST /api/runs,
           optional scheduling via bronson.workflowSchedule.v1 in the web UI, promote from examples/.

      If markers are missing, write .dream/proposed/draft-raw.txt with the full draft_agent_workflows text.
