...
# Prerequisites (apps/orchestrator/.env):
#   ALLOW_SHELL_TOOL=true
#   TOOL_SHELL_CWD=<absolute path to your project root>
...
jobs:
  ...
  scan_memory:
    prompt: |
      You are reading the existing project memory store.

      Use the shell tool to read the project's memory files. Try each command
      in order and stop once you get meaningful content:
  ...
  scan_project_docs:
    prompt: |
      You are collecting project documentation and directory context.

      Use the shell tool to gather structural context in order:

        ls -la
        find . -maxdepth 3 -name "*.md" -not -path "*/node_modules/*" \
               -not -path "*/.git/*"
        cat README.md
        find docs -maxdepth 2 -name "*.md" 2>/dev/null | xargs cat 2>/dev/null || true
  ...
  analyze_code_structure:
    prompt: |
      You are analyzing the CODE STRUCTURE knowledge plane for Dream consolidation.
      ...
      Tag each fact [HIGH] or [LOW]. Omit speculation. Target ≤ 400 words.
  ...
  analyze_product_intent:
    prompt: |
      You are analyzing the PRODUCT INTENT knowledge plane.
  ...
  consolidate:
    prompt: |
      You are the Dream consolidation engine. Synthesize the three knowledge-plane
      analyses from context into ONE canonical Markdown memory document.
  ...
  draft_agent_workflows:
    prompt: |
      ...
      no human gates, non-destructive. Prefer workspace_write. Models must be valid CLōD ids.
  ...
  emit_artifacts:
    prompt: |
      Context contains:
        ...
        - draft_agent_workflows → text with <<<YAML_MEMORY_HYGIENE>>> ... <<<END_YAML_MEMORY_HYGIENE>>>
      ...
      Steps:
        ...
        2) From draft_agent_workflows text, extract raw YAML between the MEMORY_HYGIENE markers only.
           workspace_write path .dream/proposed/memory-hygiene.yaml with that YAML.

        3) Extract raw YAML between the CAPABILITY markers only.
           workspace_write path .dream/proposed/capability-build.yaml with that YAML.

        4) workspace_write path .dream/proposed/README.md — 8–12 lines: what each file is for,
           and "Run via POST /api/runs or copy into examples/ and Load dream-state–style presets."

      Shell once only if mkdir needed: mkdir -p .dream/proposed

      If markers are missing, workspace_write .dream/proposed/draft-raw.txt with the full
      draft_agent_workflows output for manual recovery.

      End with:
        DREAM COMPLETE — artifacts under .dream/
    depends_on: [consolidate, draft_agent_workflows]
    tools: [workspace_write, shell]
    tool_rounds_max: 20
    gate: auto
    on_failure: retry
